...
void drawInvertedPendulumWorld(){
   ...
   //~ setcolor(YELLOW);
	...
	outtextxy((deviceBoundary.x2 - textwidth("START-UP CODE by:  n.h.reyes@massey.ac.nz")), deviceBoundary.y2 + textheight("H"),"START-UP CODE by:  n.h.reyes@massey.ac.nz  (C) Massey University 2022");
}
    ...
    bool done = false;
        ...
        while(!done){
			...
			while(GetAsyncKeyState(VK_ESCAPE) == 0) {
				 ...
				 //Check if external force was applied using mouse click
				 if (mousedown()) {

					int button = whichmousebutton();
					if (button == LEFT_BUTTON) {
						 externalForce = -15.0;
						 cout << "\nApplying external force = -15N." << endl;
		         ...
		         cout << "newState.angle_dot = " << newState.angle_dot << endl;
						 ...
						 cout << "inputs: " << inputs[0] << ", " << inputs[1] << endl; // for checking
						 cout << "Force: " << prevState.F << endl; // for checking
						 cout << "X input: " << inputs[INPUT_X] << endl; // for checking
						 cout << "Y input: " << inputs[INPUT_Y] << endl; // for checking
